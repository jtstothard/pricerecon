...
| `box` | Baseline probe of `https://www.box.co.uk/search?search=RTX%205070` returned HTTP 404 with no product cards. A current read-only probe receives Cloudflare HTTP 403, and no alternate route has been verified to return product HTML. | `disabled` |

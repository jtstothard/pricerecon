...
| `ccl` | Direct HTTP returns Cloudflare HTTP 403 domain-wide. Byparr anti-bot route times out (>120s) on CCL. AO/Scan/Box work via Byparr, confirming infrastructure health. CCL hardened protection exceeds current bypass capabilities. See diagnosis task t_6d1ce628. | `bot_blocked` |

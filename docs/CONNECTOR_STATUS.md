...
|| --- | --- | --- |
|| `ao` | Direct HTTP returns Cloudflare HTTP 403 challenge. Byparr/FlareSolverr to `/uk/search?search={query}` returns HTTP 404 AO error page. Both routes exhausted; no working discovery path exists. See diagnosis task t_3f02d0ca. | `disabled` |
|| `aria` | `https://www.aria.co.uk/` serves a customer-closure message; the former search route is retired. | `disabled` |
|| `ccl` | Direct HTTP returns Cloudflare HTTP 403 domain-wide. Byparr anti-bot route times out (>120s) on CCL. AO/Scan/Box work via Byparr, confirming infrastructure health. CCL hardened protection exceeds current bypass capabilities. See diagnosis task ***REMOVED***. | `bot_blocked` |

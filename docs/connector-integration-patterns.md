...
class FlareSolverrClient:
    def __init__(self, endpoint: str, timeout: float = 90.0):
        """
        Args:
            endpoint: FlareSolverr API URL (e.g., "http://***REMOVED***:8191/v1")
...
```
Connector → POST http://***REMOVED***:8191/v1
...
flaresolverr_url: "http://***REMOVED***:8191/v1"
...
class MySPAConnector(BaseConnector):
    def __init__(self, camofox_url: str | None = None):
        self.camofox_url = camofox_url or "http://***REMOVED***:9377"
...
Connector → BrowserClient (uses Camofox if camofox_url set)
         ↓
    POST http://***REMOVED***:9377/tabs
    Payload: {"userId": "...", "sessionKey": "...", "url": "https://..."}
         ↓
Camofox → Launch browser → Navigate → Render JS
         ↓
    GET http://***REMOVED***:9377/tabs/{tabId}/snapshot?format=text
         ↓
...
async def test_flaresolverr_request():
    client = FlareSolverrClient("http://***REMOVED***:8191/v1")
...
async def test_browser_client_camofox():
    config = BrowserSessionConfig(camofox_url="http://***REMOVED***:9377")

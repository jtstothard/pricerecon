...
RETRYABLE = frozenset(
    {FailureCategory.BLOCKED, FailureCategory.TIMEOUT, FailureCategory.BACKEND_UNAVAILABLE}
)
...
def run_case(
    *,
    ...
    connector_config: Mapping[str, object] | None,
    probe: Probe,
) -> list[MatrixRow]:
    """Run a bounded selection case, retrying only documented retryable failures."""
    configured = cast(
        str | list[str] | None,
        (connector_config or {}).get(
            "browser_backend", (connector_config or {}).get("browser_selection")
        ),
    )
    if configured is None:
        configured = cast(
            str | list[str] | None,
            runtime_config.get("browser_default", runtime_config.get("browser_selection")),
        )
    ...
    if not backends:
        return [
            MatrixRow(
                ...
                0,
                0,
                FailureCategory.BACKEND_UNAVAILABLE.value,
                1,
                False,
            )
        ]
...
def run_matrix(
    cases: Iterable[Mapping[str, Any]], probes: Mapping[str, Probe]
    ...
    for case in cases:
        ...
        case_rows = run_case(
            retailer=str(case["retailer"]),
            connector=connector,
            runtime_config=cast(Mapping[str, object], case["runtime_config"]),
            connector_config=cast(Mapping[str, object] | None, case.get("connector_config")),

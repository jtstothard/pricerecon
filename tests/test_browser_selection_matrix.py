...
def test_retailer_override_wins_and_retry_is_limited_to_retryable() -> None:
    ...
    rows = run_case(retailer="Acme", connector="html", runtime_config=CONFIG,
                    connector_config={"browser_backend": "backup"}, probe=probe)
    assert seen == ["backup"]
...
def test_ordered_fallback_only_on_documented_retryable_failure() -> None:
    ...
    def probe(backend: BrowserBackendConfig) -> ProbeResult:
        ...
        if backend.name == "primary":
            return ProbeResult(page_outcome="blocked", failure=FailureCategory.BLOCKED, elapsed_ms=10)
        return ProbeResult(parsed_listing_count=1, elapsed_ms=12)

    rows = run_case(retailer="Acme", connector="html", runtime_config=CONFIG,
                    connector_config=None, probe=probe)
...
def test_unconfigured_connector_is_explicitly_reported() -> None:
    rows = run_case(retailer="Acme", connector="html", runtime_config={"browser_backends": CONFIG["browser_backends"]},
                    connector_config=None, probe=lambda _: ProbeResult())
